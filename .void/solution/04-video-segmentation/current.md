# 视频切片架构（Current）

## 文档定位
本文档定义视频切片子域当前生效的方案版本，并将其作为 `V1` 的阶段性里程碑文档。

它回答的是：
- 当前生效的事实是什么。
- 当前版本实际交付什么，不交付什么。
- 当前实现入口、输入输出契约和默认运行参数是什么。
- 当前已经验证到什么程度，仍有哪些边界和后续演进方向。

相关文档：
- 历史方案：[视频切片方案历史（V1）](./history/v1.md)
- 验证索引：[视频切片 V1 验证索引](../../validation/video-segmentation-v1-validation-index.md)

## 当前生效事实
当前生效版本为 `V1`。

当前实现入口：
- [tools/rederive_video_segmentation.py](/Users/pataphaw/Projects/toward-nadal/tools/rederive_video_segmentation.py:1)

当前版本的现实形态是：
- `FFmpeg-first` 的本地重推切片器。
- 以低成本运动信号作为主候选证据，音频主要用于补边界和补置信度。
- 采用 `point-first` 路线，先把回合主片段做稳，再补齐 `break` 与完整时间轴。

当前阶段的里程碑结论是：
- 该版本已经形成可重复执行、可验证、可继续迭代的本地切片闭环。
- 当前可用性主要建立在“优先保护回合完整性”，而不是“已经把边界切得足够干净”。

## 子域职责与非目标
视频切片层的职责，是把单个长视频转换成一组按时间顺序组织、可被后续精选和分析消费的片段。

这里解决的是：
- 长视频中的回合与停顿如何被结构化。
- 切片结果如何保留时间顺序和回溯能力。
- 如何在本机环境下先跑通一个可迭代的切片闭环。

这里不解决的是：
- 哪些片段最值得送去分析。
- 如何根据片段直接生成技战术结论。
- 云端训练、重模型边界检测或跨平台部署细节。
- 任意形式的 `compact clip`、精选集或聚合导出。

## 问题定义
原始输入通常是比赛或训练的长视频。单个视频中既包含回合，也包含大量非击球时段，例如：
- 发球前准备。
- 回合间停顿。
- 捡球和走位。
- 换边和休息。

本子域的长期目标，不是把视频压成“更紧凑的精彩片段”，而是把连续视频流转换成一条可管理的时间轴。

因此，目标产物需要满足以下原则：
- 切片后仍然保留完整时间顺序。
- 长期目标是不丢内容，片段拼接后能够还原原视频。
- 回合语义重要，但完整性优先于紧凑性。
- 短暂停顿可以吸收到相邻回合片段中。
- 较长的休息、换边或死球区间应能够独立成段。

当前阶段的策略重点不是一步到位实现完整时间轴覆盖，而是先压住两类高成本错误：
- 同一回合被切碎。
- 画面仍有明显运动，但因为音频证据弱而被误删。

## 输出模型与当前交付范围
方案层定义的目标产物是统一的 `segment clip` 时间轴模型。

`segment clip` 至少包含两类语义：
- `point`：以一个回合为核心的片段。
- `break`：较长的休息、换边、长死球或其他不应吸收到相邻回合中的区间。

这意味着切片层最终应输出的是一条连续时间轴，而不是只输出“值得保留的回合”。

在演进节奏上，当前采用 `point-first` 路线：
- 先把 `point` 级片段做稳。
- 再补齐 `break` 片段与全时间轴覆盖。

当前阶段的现实交付仍只有 `point clip`，并且明确不做：
- `compact clip`
- 多回合聚合
- 面向复盘浏览的二次拼接

## 硬约束
以下约束高于局部调参，是当前方案层的基本边界。

### 1. 只能切分，不能丢弃
切分器只能把原视频重新划分成一组片段，不能通过“静默忽略”直接丢掉区间。

这意味着：
- 死球、休息、换边等非回合内容不能被静默删除。
- 较短死球可以保留在相邻片段边缘。
- 较长死球在后续完整时间轴模型中应能够单独成段。

### 2. 禁止单回合跨片段
如果一个完整回合被切到多个片段中，视为严重错误。

当前优化优先级中：
- 避免单回合碎裂，高于把所有死球都切干净。

### 3. 允许多个紧凑回合同片段
当两个回合之间间隔极短，或者当前证据不足以可靠断开时，允许暂时保留在同一个片段中。

当前方案接受：
- “略粘”

当前方案不接受：
- “切碎一个回合”

## 执行约束
当前主方案必须服从真实本地环境，而不是假设理想训练环境。

已确认的约束是：
- 运行环境是 `macOS Apple Silicon`。
- 本地可用 `ffmpeg`、`ffprobe` 和 `python3`。
- 首版不应建立在 `CUDA`、`Linux`、重型视觉模型或训练流程之上。

由此得到的方案约束是：
- 主路径必须能够在本机直接跑通。
- 主路径优先采用低依赖、强可解释的切片方法。
- 增强能力可以后加，但不能反过来变成首版的运行前提。

## 当前决策
当前主方案选择 `FFmpeg-first` 的低依赖切片路径，同时明确以下判断：
- 当前主路径不是“纯音频切分”，而是低成本多信号切片。
- 冲突时优先相信运动证据，而不是由音频单独主导是否保留某段内容。
- 首版优先采用可解释的规则逻辑，而不是训练型边界检测器。
- 切点语义正确性优先于关键帧粗切的导出效率。
- 当前先把 `point` 做稳，再演进到完整 `segment clip` 时间轴。

这样选择的原因是：
- 它最符合当前本机环境和依赖现实。
- 它最先解决“手工切片太费时”的核心问题。
- 真实球场素材中，`silencedetect` 往往不足以单独承担主切分依据。
- 低成本运动信号虽然粗糙，但足以帮助修复纯音频路径的典型误删和误拆。
- 它保留了清晰的增强路径，而不会把系统锁死在某个重型方案上。

## 当前能力范围
`V1` 当前只覆盖以下能力：
- 对单个长视频做探测。
- 提取音频时间窗与低成本画面运动时间窗。
- 以“运动优先于音频”的规则构造候选区段。
- 导出 `point clip`。
- 生成 `manifest`、日志和边界证据。

`V1` 当前不覆盖：
- `break clip` 导出。
- 完整时间轴覆盖。
- 统一的 `segment clip` 模型。
- 无音频视频的可靠降级路径。
- 外部配置文件驱动的参数管理。

## 当前实现流程
### 1. 素材探测
脚本先调用 `ffprobe` 获取容器和流信息，读取视频总时长，并把结果写入 `manifest.probe`。

### 2. 音频时间窗提取
脚本通过 `ffmpeg + astats + ametadata` 生成固定窗口的音频日志，再按 `RMS` 阈值构造音频活跃区段。

当前实现特点：
- 以固定窗口扫描，而不是逐事件检测。
- 音频主要提供辅助边界证据。

### 3. 运动时间窗提取
脚本用 `ffmpeg` 抽取低帧率灰度帧，计算相邻帧差，生成逐窗运动分数。

当前实现的关键点：
- 低采样率，成本可控。
- 对中心区域做额外加权。
- 最终使用 `motion_score = max(mean_abs_diff, roi_abs_diff * roi_boost)`。

### 4. 候选区段融合
当前 `V1` 不是一个显式命名状态机，而是一套“区段融合 + 分组 + 拆分”的启发式流程。

具体做法是：
- 先把运动簇作为主候选证据。
- 再把足够接近的音频区段吸附到已有候选上。
- 对彼此间隔较短的候选做进一步合组。
- 对持续时间过长且存在足够大间隙的组做递归拆分。

### 5. 片段扩展与重叠修正
候选组在导出前会增加前后缓冲，并在相邻片段发生重叠时按中点切开，避免导出后出现互相覆盖。

### 6. 导出与落盘
当前脚本逐段调用 `ffmpeg` 导出 `point clip`，并输出以下产物：
- `manifest.json`
- 音频和运动日志
- `point_clips/` 目录

`manifest` 记录输入探测结果、参数快照、计数信息、片段列表和资源统计。

## 输入与输出契约
脚本参数如下：
- `--video`
- `--work-root`
- `--source-video-id`
- `--run-id`
- `--collect-resources`

输出目录结构为：
- `<work-root>/<source_video_id>/<run_id>/manifest.json`
- `<work-root>/<source_video_id>/<run_id>/logs/`
- `<work-root>/<source_video_id>/<run_id>/point_clips/`

当前一个已验证运行样例如下：
- 运行目录：[20260422-074627](/Users/pataphaw/Projects/toward-nadal/.work/clips/img_3026/20260422-074627)

### `manifest.json`
当前 `manifest` 顶层包含以下关键字段：
- `input`
- `backup`
- `probe`
- `params`
- `counts`
- `point_clips`
- `compact_clips`
- `logs`
- `rederived`

其中当前状态是：
- `point_clips` 为主产物。
- `compact_clips` 固定为空数组，仅作为兼容占位。

### `point clip`
每个 `point clip` 当前至少包含：
- `clip_id`
- `source_video_id`
- `start_time`
- `end_time`
- `duration`
- `confidence`
- `boundary_evidence`
- `export_path`
- `point_index`
- `child_point_clip_ids`
- `aggregation_reason`

### `boundary_evidence`
当前 `boundary_evidence` 记录的是实现层证据，而不是高层语义标签。

至少包含：
- `duration_flag`
- `evidence_sources`
- `source_intervals`

## 当前默认参数
参数当前全部硬编码在脚本的 `DEFAULT_PARAMS` 中，尚未外置。

### 音频窗口相关
- `window_seconds = 0.4`
- `active_gap_seconds = 0.6`
- `min_active_span_seconds = 2.0`
- `rms_threshold_dbfs = -32.8`

### 运动提取相关
- `motion_fps = 2.0`
- `motion_scale = 64:36`
- `motion_activity_threshold = 4.5`
- `motion_cluster_gap_seconds = 2.0`
- `min_motion_span_seconds = 3.0`

### 中心区域加权相关
- `motion_roi_left_ratio = 0.2`
- `motion_roi_right_ratio = 0.8`
- `motion_roi_top_ratio = 0.1`
- `motion_roi_bottom_ratio = 0.95`
- `motion_roi_boost = 1.35`

### 合并与拆分相关
- `motion_audio_join_gap_seconds = 3.0`
- `fragment_gap_seconds = 12.0`
- `motion_fragment_gap_seconds = 15.0`
- `fragment_short_point_seconds = 14.0`
- `fragment_max_combined_duration = 90.0`
- `force_split_above_seconds = 45.0`
- `split_gap_seconds = 4.5`
- `motion_split_gap_seconds = 20.0`

### 导出相关
- `pre_roll_seconds = 1.2`
- `post_roll_seconds = 1.4`
- `min_point_duration = 5.0`
- `max_point_duration = 35.0`

这些参数当前的现实作用是：
- 优先减少误拆和误删。
- 容忍更长的 `point clip`。
- 用可解释阈值替代学习型边界判断。

## 当前验证状态
截至 `2026-04-24`，与 `V1` 直接相关的验证记录包括：
- [2026-04-20-video-segmentation-v1-bootstrap](../../validation/2026-04-20-video-segmentation-v1-bootstrap.md)
- [2026-04-21-video-segmentation-replay-benchmark](../../validation/2026-04-21-video-segmentation-replay-benchmark.md)
- [2026-04-22-video-segmentation-boundary-rederive](../../validation/2026-04-22-video-segmentation-boundary-rederive.md)
- [2026-04-22-video-segmentation-motion-priority-fix](../../validation/2026-04-22-video-segmentation-motion-priority-fix.md)
- [2026-04-24-img3076-segmentation-sampling](../../validation/2026-04-24-img3076-segmentation-sampling.md)

当前已确认事实：
- “运动优先于音频”的调整已经落地。
- 在 `img_3026` 的 `20260422-074627` 运行中，脚本导出了 `43` 个 `point clip`，覆盖率约为 `74.05%`。
- 原先被错误拆开的 `28/29` 典型案例，已在修复后被合并保留。
- 在 `IMG_3076` 的抽样复核中，大多数抽样边界未见明显断回合，但 `point-023 -> point-024` 仍是一处可疑误拆。
- 当前主矛盾已经从“误删和误拆”进一步转向“多个独立回合与较长无效内容被吸收到同一片段中”。
- 当前导出保持 `1920x1080`，但存在重新编码带来的额外压缩损失；它会影响观感，但不是当前切片逻辑的主问题。

## 当前已知边界
### 1. 仍不是完整时间轴方案
当前导出物仍然只覆盖被判定为 `point` 的区段，尚未补齐 `break` 区段，因此不能无缝拼回原视频。

### 2. 实现形态仍偏启发式
当前逻辑核心是区段融合与阈值拆分，不是显式的回合语义状态机。

### 3. 长片段处理仍偏保守
当前规则对长片段更多是降低置信度，而不是稳定地做二次细分，因此仍可能导出明显偏长的片段。

### 4. 参数仍散落在代码中
当前参数没有独立配置文件，也没有按运行版本做显式配置管理，不利于长期回归和跨样本比较。

### 5. 无音频降级未落地
方案层要求为无音频或音频极差场景设计降级路径，但当前脚本还没有形成可靠实现。

## 历史关联
当前 `V1` 的参数取向和边界偏好，不应脱离其历史演进去理解。

需要回看历史时，应优先参考：
- [视频切片方案历史（V1）](./history/v1.md)

历史文档主要解释三件事：
- 为什么方案从“音频优先”转向“运动优先”。
- 为什么当前明确接受“略粘”，但不接受“断回合”。
- 为什么 `compact clip` 已经退出当前主方案。

## 后续演进方向
建议按以下顺序推进：

1. 先把 `segment clip` 契约补齐，加入 `break` 并实现全时间轴覆盖。
2. 把参数从 `DEFAULT_PARAMS` 外置，形成可记录、可比较的运行配置。
3. 对超长 `point clip` 增加二次细分策略，而不是只降置信度。
4. 为无音频或极差音频场景补充明确降级路径。
5. 固化一组回归样本，把每次规则调整都落到 `validation` 证据中。

## 与 05 selection v1.0.1 的关系
05 selection 已升级为 `CV 候选生成 + 本地视觉模型 rerank`。

这不会改变 04 当前职责：
- 04 仍负责生成 `point_clips` 与边界证据。
- 04 仍允许“略粘”，优先避免单回合被切碎。
- 04 不需要先提供完美 rally 边界，才能被 05 消费。

05 v1.0.1 会把每个 `point_clip` 视为候选容器，在内部生成多个 `cv_candidate`，再由本地视觉模型判断是否 `multi_rally`、是否完整、是否值得分析。

因此 04 后续优化仍应围绕切片时间轴质量展开，不应为了迁就 05 而在本层提前做代表性选择或语义 rerank。
